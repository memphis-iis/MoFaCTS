...
The Creator title is deterministic rather than a second AI request: it combines the interpretation stage's existing structured `subject`, the authoritative extracted-entry count, and the selected Learning/Test mode. The learner-facing lesson name retains ordinary spaces. Package, TDF, stimulus, and media filenames are sanitized separately; trailing or repeated periods are removed before the `.zip` extension so authored punctuation cannot trigger the traversal guard.
