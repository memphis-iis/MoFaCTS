...
const { getImportContent } = require('./module-imports');
  ...
  Object.entries(moduleFiles).forEach(([filename, defaultContent]) => {
    ...
    if (fs.existsSync(filePath)) {
      ...
      if (!existing.includes(defaultContent)) {
        try {
          writeFileAtomically(filePath, defaultContent);
        } catch (err) {
...
export function cleanBuildContextFiles() {
  ...
  try {
    ...
    // The development watchers consume main-dev entry files continuously.
    // Preserve that directory across Meteor plugin reinitialization so an old
    // watcher never observes a delete/recreate gap. Cold-start output reset is
    // owned by deploy/hotfix-local.ps1 after the complete process tree stops.
    const shouldPreserveMainDevelopmentContext = isMeteorAppDevelopment() && !isMeteorAppNative();
    const uniqueDirPaths = new Set([
      ...(!shouldPreserveMainDevelopmentContext ? [mainClientPath, mainServerPath] : []),
      testModulePath,
      testClientPath,
      testServerPath,
    ]);
