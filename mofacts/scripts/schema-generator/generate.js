 ...
 * Usage:
 *   cd mofacts
 *   node scripts/schema-generator/generate.js [path-to-mofacts_config]
 *
 ...
 * This captures all possible fields from all TDFs
 */
...
async function main() {
  ...
  // Merge all TDFs into one deep-merged object to capture all possible fields
  
  const mergedTdf = deepMerge(tdfs);
  

  
  const schema = inferSchema(mergedTdf);
