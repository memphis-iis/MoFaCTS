...
function copyRun(run: AiContentPipelineRun): AiContentPipelineRun {
  return {
    ...
    ...(run.tableRun ? { tableRun: { ...run.tableRun } } : {}),
...
async function executeAiStage<T>(
  ...
  try {
    ...
    finishTrace(run, options, trace, 'succeeded', parsedContent);
    return { parsedContent, result, trace };
  } catch (error) {
    trace.response = {
...
function tableStrategy(intent: AiContentIntent): AiContentTableStrategy | null {
  return intent.textPairingStrategy === 'generated-table' || intent.textPairingStrategy === 'provided-table'
    ? intent.textPairingStrategy
    : null;
}
...
async function createTableContent(
  ...
  const settings = stageSettings(options, 'generate-table');
  const execution = await executeAiStage(
    run,
    options,
    'generate-table',
    buildGeneratedTablePrompt(options.notes, intent, settings.instructions),
    generatedTableSchema(intent.expectedItemCount ?? null),
    `mofacts_ai_content_table_v${AI_CONTENT_CONTRACT_VERSION}`,
    {
      strategy,
    ...
    (value) => validateGeneratedTable(value, intent.expectedItemCount ?? null),
  );
  const generation = execution.parsedContent;
  const pairs = generation.pairs.map(({ prompt, response }, index): AiContentPair => ({
    id: `table-${run.runId}-${index + 1}`,
    kind: 'text',
    stimulus: prompt,
    response,
    provenance: {
      sourcePath: strategy,
      sourceLocator: `${strategy === 'provided-table' ? 'Author-supplied' : 'AI-generated'} table row ${index + 1}`,
      generationTraceId: execution.trace.traceId,
    },
  }));
  run.tableRun = {
    strategy,
    scopeSummary: generation.scopeSummary,
    expectedItemCount: intent.expectedItemCount ?? null,
    actualItemCount: pairs.length,
    generationTraceId: execution.trace.traceId,
  };
  run.resolutions = pairs.map((pair) => ({
    itemId: pair.id,
    response: pair.response,
    promptType: 'text',
    sourcePath: strategy,
    prompt: pair.stimulus,
  }));
  publish(run, options);
  const warning = strategy === 'provided-table'
    ? 'AI reformatted the author-supplied table. Review the light cleanup and column mapping before saving.'
    : 'This table was generated by AI and was not verified against an external source. Review its factual accuracy before saving.';
  return { run, pairs, assets: [], warnings: [warning] };
}
...
function createSourceFieldMappedItems(
  ...
  extracted.forEach(({ entry, prompt, response }) => {
    ...
    const sourcePath: Exclude<AiContentItemResolution['sourcePath'], AiContentTableStrategy> = issue
      ? 'unresolved'
...
export async function runAiContentPipeline(options: AiContentPipelineOptions): Promise<AiContentPipelineResult> {
  ...
  const selectedTableStrategy = tableStrategy(intent);
  if (selectedTableStrategy) {
    return createTableContent(run, options, intent, selectedTableStrategy);
  }
