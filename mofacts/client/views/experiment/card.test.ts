...
declare const Stims: any;
declare const Histories: any;
declare const GlobalExperimentStates: any;
declare const DynamicAssets: any;
type RestorableFn = (() => unknown) & { restore(): void };
  ...
  describe('card.js - Smoke Tests', function() {
    ...
    beforeEach(function() {
      ...
      StubCollections.stub([
        Tdfs, Stims, Histories,
        GlobalExperimentStates, DynamicAssets
      ]);

      // Clear all Session keys
      const sessionAny = Session as unknown as { keys?: Record<string, unknown> };
      Object.keys(sessionAny.keys || {}).forEach((key) => {
      ...
      sinon.stub(Meteor, 'userId').returns(Meteor.user()?._id || Random.id());
    });

    afterEach(function() {
      // Restore stubs
      StubCollections.restore();
      (Meteor.user as RestorableFn).restore();
      (Meteor.userId as RestorableFn).restore();
    ...
    describe('Smoke Test 1: Basic Trial Flow (Text Question)', function() {
      ...
      it('should handle answer submission without crashing', function(done) {
        ...
        // Mock Meteor method
        const callStub = sinon.stub(MeteorAny, 'callAsync').resolves({
          isCorrect: true,
          feedback: 'Correct!'
        });
    ...
    describe('Smoke Test 2: Multiple Choice Trial', function() {
      ...
      it('should set up button trial without errors', function(done) {
        ...
        expect(buttonList.filter((b: { isAnswer?: boolean }) => b.isAnswer)).to.have.length(1);
