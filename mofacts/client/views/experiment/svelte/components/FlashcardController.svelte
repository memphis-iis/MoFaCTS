<script>
  ...
  async function runInteractionHandoff(nextKind) {
    ...
    clientConsole(2, '[FlashcardController][InteractionTransition] fade-out:start', {
    ...
    clientConsole(2, '[FlashcardController][InteractionTransition] feedback-fade-in:start', {
      to: nextKind,
      durationMs,
      subsetKind,
    });
    ...
    if (token === interactionTransitionToken) {
      clientConsole(2, '[FlashcardController][InteractionTransition] feedback-fade-in:end', {
        to: nextKind,
        durationMs,
        subsetKind,
      });
