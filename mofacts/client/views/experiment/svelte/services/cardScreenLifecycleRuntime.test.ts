...
import { createCardScreenLifecycleRuntime } from './cardScreenLifecycleRuntime';
...
function machineSnapshot(value: unknown): CardMachineSnapshot {
  return {
    value,
    context: {},
...
function createHarness(overrides: Partial<{
  ...
  const options = {
    ...
    setState: (snapshot: CardMachineSnapshot) => events.push(`state:${String(snapshot.value)}`),
  ...
  return {
    controller: createCardScreenLifecycleRuntime(options),
    events,
  };
}
