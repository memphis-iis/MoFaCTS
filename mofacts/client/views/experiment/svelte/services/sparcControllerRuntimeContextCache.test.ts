...
function display(productionRules: SparcControllerDisplay['productionRules'] = []): SparcControllerDisplay {
  return {
    documentId: 'doc-1',
    nodes: [],
    productionRules,
...
describe('sparcControllerRuntimeContextCache', function() {
  afterEach(function() {
    clearSparcControllerRuntimeContextCache();
  });
  ...
  it('reuses the authored document for the same display signature', function() {
    ...
    const secondContext = getSparcControllerRuntimeContext({
      TDFId: 'tdf-1',
      sessionID: 'session-1',
      documentId: 'doc-1',
      display: display(),
  ...
  it('advances cached replay state with newly retained records', function() {
    ...
    const advancedContext = getSparcControllerRuntimeContext({
      TDFId: 'tdf-1',
      sessionID: 'session-1',
      documentId: 'doc-1',
      display: display(),
  ...
  it('rebuilds the authored document when the display signature changes', function() {
    ...
    const changedContext = getSparcControllerRuntimeContext({
      TDFId: 'tdf-1',
      sessionID: 'session-1',
      documentId: 'doc-1',
