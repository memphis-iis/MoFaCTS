...
export async function resumeFromExperimentState(_initialTdfFile: unknown): Promise<SvelteCardInitResult> {
  ...
  try {
    ...
    let UIsettings: UiSettingsMap = Object.keys(mergedUiSettings).length ? mergedUiSettings : {};

    const displayPresets: Record<string, UiSettingsMap> = {
      default:{
    ...
    if (Object.keys(UIsettings).length > 0) {
      for(const key in displayPresets['default']){
        if(!Object.prototype.hasOwnProperty.call(UIsettings, key)){
    ...
    // Layout from stimuliPosition
    switch(UIsettings.stimuliPosition){
      case 'top':
        UIsettings.choiceColWidth = 'col-12';
        UIsettings.displayColWidth = 'col-12';
        UIsettings.responseColWidth = 'col-12';
        UIsettings.textInputDisplay = "";
        UIsettings.textInputDisplay2 = "";
        break;
      case 'left':
        UIsettings.choiceColWidth = 'w-100';
        UIsettings.displayColWidth = 'col-12 col-lg-6';
        UIsettings.responseColWidth = 'col-12 col-lg-6';
    ...
    // displayUserAnswerInFeedback switch
    ...
    // instructionsTitleDisplay switch
    ...
    UiSettingsStore.set(sanitizeUiSettings(UIsettings || {}, { tdfName }));
