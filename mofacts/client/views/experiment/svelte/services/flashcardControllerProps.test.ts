...
describe('flashcard controller props adapter', function() {
  ...
  it('builds FlashcardController props and blocker sources for an active question slot', function() {
    const result = buildFlashcardControllerProps({
  ...
  it('keeps image stimulus blockers while the first active question is hidden for reveal', function() {
    ...
    const result = buildFlashcardControllerPropsFromSubset({
      buttonList: [],
      correctAnswer: 'Answer',
  ...
  it('builds FlashcardController props from an already-derived active subset', function() {
    const subset = buildTrialSubset({
      kind: 'feedback',
      display: { text: 'Prompt' },
      displayVisible: true,
