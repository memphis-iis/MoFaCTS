...
describe('card wake lock controller', function() {
  ...
  it('requests once while active and visible', async function() {
    ...
    expect(harness.logs.map((entry) => entry.message)).to.include('[ContentSurface] Screen wake lock acquired (initial)');
  });
  ...
  it('releases when inactive and removes the release listener', async function() {
    ...
    expect(harness.logs.map((entry) => entry.message)).to.include('[ContentSurface] Screen wake lock released by app (inactive)');
  });
  ...
  it('clears active state when the browser releases the sentinel', async function() {
    ...
    expect(harness.controller.hasActiveWakeLock()).to.equal(false);
    expect(harness.logs.map((entry) => entry.message)).to.include('[ContentSurface] Screen wake lock released (manual)');
  });
  ...
  it('logs request and release failures without throwing', async function() {
    ...
    expect(requestFailure.logs[0]!.message).to.equal('[ContentSurface] Screen wake lock request skipped (denied)');
    ...
    expect(releaseFailure.logs.map((entry) => entry.message))
      .to.include('[ContentSurface] Screen wake lock release failed (destroy)');
  });
});
