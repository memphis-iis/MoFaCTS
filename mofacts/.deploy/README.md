...
- sets `HOME` to `mofacts/.deploy/local-data` so dynamic assets and H5P content resolve to the same local data folders as the container workflows,
- writes logs and a PID file to ignored local state under `mofacts/.deploy/local-dev/`,
