...
- AI creation starts with author notes, a Learning/Test selector, and Submit. One run uses either text prompts or image prompts; correct responses are always text. Review contains a required editable title and one row for every item found on the selected source list.
- AI first interprets the request as one concise Wikipedia list-search intent. Wikipedia search returns at most three real page candidates, and AI may select only one supplied opaque candidate ID. Definition and image runs select one structural table, list, or gallery on that page; source-field-mapping runs consider only table regions exposing at least two fields. AI never supplies an item, page, link, URL, or Wikimedia filename. After one individually validated image agrees with a response-bearing canonical filename found on another authoritative item page, MoFaCTS may predict later `File:` titles and use only those that Wikimedia resolves canonically. Text-pair stimuli are learner-visible. An image-pair stimulus is exactly `image: <response>` and is never learner-visible. MoFaCTS supplies IDs, typed-response settings, lesson structure, defaults, and package contents deterministically.
...
- Text runs support two strategies. Definition runs make one strict definition request per source entry; the source entry supplies the response, and a definition that reveals that response is rejected without removing the item from review. Source-field-mapping runs preserve the selected table's headings and row values, make one bounded decision selecting supplied prompt and response field IDs, and then build every pair deterministically. For example, a state-capitals request can map `State` to `Capital` without generating fifty definitions. Empty fields and duplicate prompts remain unresolved for review.
...
- Administrators use one **AI Content Prompt Lab** in Admin Tests. Its proven Admin capability lookup and explicit stage caller remain fixed while the Creator runs the same full orchestrator and request semantics through its own scoped adapter. The Lab exposes editable author notes plus per-stage system instructions, user instructions, reasoning, output budget, and strict schema. **Reset to code defaults** first checkpoints the current draft, then restores every stage setting from the Creator defaults while preserving author notes and saved checkpoints. Its trace shows effective non-secret model requests, Wikipedia/Wikimedia URLs and candidate objects, direct/detail branches, decisions, acquisition and conversion results, unresolved reasons, and the final Creator-style review. Retrieved page HTML is represented by metadata rather than embedded repeatedly; the complete run object is rendered only when an administrator requests a snapshot, so a growing image run remains scrollable. A stage can be retried from its recorded input while validated upstream AI outputs are reused and downstream objects are rebuilt. Drafts and up to 30 named checkpoints remain in the current browser; provider results are not retained on the server. The Creator derives a concise, space-preserving lesson title from the existing structured subject and authoritative extracted item count, keeps its own authoring/review/save interface, and reports the current shared pipeline stage and item in its green running-status region. Package filenames remain independently sanitized.
