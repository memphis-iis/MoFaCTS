...
- Current admin tests surface: `/admin/tests` is already an admin-only route named `client.adminTests`, rendering the `testRunner` template. Today it is mainly a Svelte Card Layout Tester shell, so readiness work should add deployment tests/diagnostics there instead of exposing readiness publicly.
