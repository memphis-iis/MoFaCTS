...
The safe-expression rollout has a mandatory compatibility stage. Stage 1 validates author-provided `calculateProbability` formulas and `adaptiveLogic` rules as bounded data at every write boundary, while production execution and CSP temporarily retain their existing behavior. Run the admin Deployment Readiness check and require `tdf.expressions` to pass; it scans live units and unit templates in bounded batches and reports only counts plus bounded TDF IDs/field paths—not formula text or learner data. Rewrite any invalid content into the documented deterministic mathematical subset before Stage 2. Only then may Stage 2 switch runtime evaluation and remove `unsafe-eval`; invalid authored formulas have no legacy execution or default-model substitution after that cutover.
