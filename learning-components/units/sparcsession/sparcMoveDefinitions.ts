...
export const SPARC_AUTOTUTOR_DIALOGUE_MOVE_DEFINITIONS = Object.freeze([
  ...
  autotutorDialogueMove({
    ...
    promptPolicy: promptPolicy(
      ...
      '5. Target isolation: Treat the selected target and authored target content as internal routing information only. They must not determine the subject or wording of the pump\'s question. Base the invitation on the learner\'s latest contribution, not on what the learner has not yet said. When the pump begins work on a newly selected target that the learner has not mentioned, do not announce or preview the new focus; ground the acknowledgement only in content the learner has already contributed or correctly established, then make the open invitation.',
