...
export const SPARC_AUTOTUTOR_DIALOGUE_MOVE_DEFINITIONS = Object.freeze([
  ...
  autotutorDialogueMove({
    ...
    promptPolicy: '**Scaffold locally: use dialogue history diagnostically to infer the learner\'s current ZPD, make the relevant task structure visible, and keep the main reasoning step with the learner.** Give brief correctness or quality confirmation. Use dialogue history to make the feedback contingent: when the learner\'s current answer resolves an earlier gap, misconception, or incomplete relation, briefly mark that progress. Do not add new substantive content, extend the answer, or open a new line of reasoning. Avoid repeating the same praise formula used in the immediately prior tutor turn.',
  }),
  autotutorDialogueMove({
  ...
  autotutorDialogueMove({
    ...
    promptPolicy: '**Scaffold locally: use dialogue history diagnostically to infer the learner\'s current ZPD, make the relevant task structure visible, and keep the main reasoning step with the learner.** Clearly but politely indicate that the answer is incorrect, off-target, or not addressing the current expectation. Use dialogue history to distinguish a first miss from repeated non-uptake. For a first miss, keep the correction brief and general. For a repeated miss, do not simply say "not quite" again; redirect attention to the overlooked task demand, constraint, contrast, or kind of answer needed. Avoid over-explaining and do not replace the learner\'s reasoning with a full solution.',
  }),
  autotutorDialogueMove({
  ...
  autotutorDialogueMove({
    ...
    promptPolicy: '**Scaffold locally: use dialogue history diagnostically to infer the learner\'s current ZPD, make the relevant task structure visible, and keep the main reasoning step with the learner.** Redirect the learner away from an unproductive or partially wrong idea without harsh evaluation. Use dialogue history to determine whether this is a new wrong path or a recurring one. If it is new, give a light constraint or reframing. If it is recurring, make the conflict with the target structure more visible by pointing to the relevant contrast, criterion, or task demand. Do not give a full correction unless the selected move and authored content permit it.',
  }),
