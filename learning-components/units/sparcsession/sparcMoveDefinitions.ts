...
export const SPARC_AUTOTUTOR_DIALOGUE_MOVE_DEFINITIONS = Object.freeze([
  ...
  autotutorDialogueMove({
    ...
    promptPolicy: promptPolicy(
      ...
      '4. Named-concept focus: Explicitly identify the active concept before the open invitation. Derive one short, natural, learner-facing concept name from the selected authored target content; do not expose an internal id or recite the full target proposition. For example, use a phrase such as "the balance used to calculate interest" rather than an opaque target key.',
      ...
      '8. Semantic openness check: Before returning tutorMessage, verify that the pump introduces only the short active-concept name needed to establish focus. It must not introduce a missing relationship, comparison, time dimension, consequence, or calculation. The invitation must remain valid regardless of which part of the named concept the learner chooses to elaborate. If it points toward a particular missing dimension, rewrite it as a genuinely open invitation.',
