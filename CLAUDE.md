...
- Debugging via server logs (SSH access to staging server)
- Reading/analyzing server logs with `docker logs` commands
- Investigation and troubleshooting

**User's Role:**
- Docker builds and deployments (see wiki runbooks)
- Docker image pushes to Docker Hub (`docker push`)
- Final deployment decisions

**Rationale:** The user has the Docker environment properly configured locally and is familiar with the build process. Claude can SSH to the server to check logs and investigate issues, but the user handles all Docker build/push operations.

---

## Workspace Structure

This project consists of **three separate Git repositories** located in the workspace:

### 1. Main Application Repository
- **Location:** `C:\Users\ppavl\OneDrive\Active projects\mofacts`
- **Remote:** https://github.com/memphis-iis/mofacts.git
- **Purpose:** Core MoFaCTS application code (Meteor.js app)
- **Contains:** Client/server code, packages, documentation, Docker configs

### 2. Configuration/Content Repository
- **Location:** `C:\Users\ppavl\OneDrive\Active projects\mofacts_config`
- **Remote:** https://github.com/Optimal-Learning-Lab/mofacts_config.git
- **Purpose:** TDF packages, course content, stimulus files, experiment definitions
- **Contains:** TDF XML files, stim files, course materials, experiment configs

### 3. Wiki Repository
- **Location:** `C:\Users\ppavl\OneDrive\Active projects\mofacts.wiki`
- **Remote:** https://github.com/memphis-iis/mofacts.wiki.git
- **Purpose:** Project wiki documentation
- **Contains:** Wiki pages, additional documentation

**Important:** When working on content (TDFs, stim files, course materials), you'll be working in the **mofacts_config** repository, NOT the main mofacts repository!
...
### Core Framework
- **Meteor.js 3.4** - Full-stack JavaScript framework (recently upgraded from 1.12)
...
- **Status:** Stable (see `docs/SPEECH_RECOGNITION_STATE_MACHINE.md`)
...
### 10. **Svelte Card Layout System** (see `docs/SVELTE_CARD_LAYOUT_SPEC.md`)
- Flex-based responsive layout for learning trials
- **Image stimuli**: 3:1 ratio (stimulus:interaction) on ALL screen sizes
- **Text stimuli**: 1:1 ratio (equal split)
- Interaction content can push stimulus smaller (never clips)
- **CRITICAL**: Do not override flex ratios in media queries without understanding the full spec
...
1. **Meteor 1.12 → 3.3.2** - Massive upgrade (see `docs/METEOR_3.0_UPGRADE_GUIDE.md`)
   - Node 12 → Node 22
   - Async/await migration (500+ MongoDB operations)
   - Express 5 (from Connect)
   - MongoDB driver 6.x (promises only)

2. **Security Hardening** (see `docs/SECURITY_AUDIT_REPORT.md`)
   - XSS prevention (DOMPurify)
   - Method protection layer
   - Input validation
   - Audit logging

3. **Performance Optimization** (see `docs/SERVER_OPTIMIZATION_PHASE1.md`)
...
The `docs/` directory contains **50+ detailed documentation files** covering:
...
**Always check `docs/` for context before making changes!**
...
- **Logging:** ALWAYS use `serverConsole` (server-side) or `clientConsole` (client-side). NEVER use raw client `console.log`, `console.info`, `console.warn`, `console.error`, or other console methods directly.
- **Client logging contract:** `clientConsole(level, ...)` in `svelte-app/mofacts/client/lib/clientLogger.ts` is the canonical gate for browser logging, and it is controlled by the admin-configured `clientVerbosityLevel` surfaced in `svelte-app/mofacts/client/views/adminControls.ts`. If admins turn client logging off, routine browser console output should disappear.
- **Implication for AI edits:** Do not add always-on client instrumentation for router traces, session cleanup, card-machine state transitions, fade timing, SR traces, or similar diagnostics. Route them through `clientConsole(...)` with an appropriate verbosity level so the admin toggle can suppress them.
...
1. **Client-side console:**
   ...
   - For application logging, prefer existing `clientConsole(...)` output over raw browser `console.*` traces
...
4. **State machine debugging:**
   ...
   - See `docs/STATE_MACHINE_TRACING_GUIDE.md`
...
Canonical deployment and build procedures live in the wiki:

- Remote production runbook: https://github.com/memphis-iis/mofacts/wiki/Remote-Install
- Building from source: https://github.com/memphis-iis/mofacts/wiki/Building-Mofacts-using-Docker-Compose
...
### Server Setup
- **OS:** Ubuntu 18.04+ (24.04 recommended, see `docs/UBUNTU-24.04-UPGRADE-GUIDE.md`)
...
### Security
- **XSS Risk:** Many places use `.html()` - audit ongoing (see `docs/INNERHTML_AUDIT_REPORT.md`)
- **Method Protection:** Not all methods have protection registered
- **Input Validation:** Inconsistent use of `check()` package

### Speech Recognition
- **Complex State Machine:** See `docs/SPEECH_RECOGNITION_STATE_MACHINE.md`
...
- **Local Meteor Cache:** Do **not** run `meteor` locally; it drops `mofacts/.meteor/local/dev_bundle` which bloats/breaks Docker build contexts (delete it if it appears).
...
- **CRITICAL: NO UNSAFE FALLBACKS** - Code must NEVER use silent failure fallbacks that hide critical errors. If a critical condition fails (e.g., unit has no cards, engine initialization fails, required data missing), the code MUST throw an explicit error with full context. Silent fallbacks that provide default behavior without logging errors are EXTREMELY DANGEROUS and hide real bugs.
  - **Example of unsafe fallback (NEVER DO THIS):**
    ```javascript
    // BAD: Returns success when schedule is empty, hiding the error
    if (schedule.q.length === 0) {
      return { unitFinished: true };  // Silent failure!
    }
    ```
  - **Correct approach (ALWAYS DO THIS):**
    ```javascript
    // GOOD: Throws explicit error with context
    if (!schedule.q || schedule.q.length === 0) {
      throw new Error(`Assessment session has no cards - check clusterlist`);
    }
    ```
  - **Where this matters most:**
    - Unit engine initialization ([unitEngine.js](mofacts/client/views/experiment/unitEngine.js))
    - Unit type detection ([svelteInit.js](mofacts/client/views/experiment/svelte/services/svelteInit.js))
    - Card selection and scheduling
    - Data validation before processing
  - **Key principle:** Better to see an explicit error and fix the root cause than to hide it with a fallback that makes the system appear to work while actually breaking user experience.
  - **Recent fixes:** Removed unsafe fallbacks in unit type detection (2025-01-09) and added validation for empty schedules/models
...
See `docs/README.md` for full documentation index. Key docs:

### Design & Architecture
- `SPEECH_RECOGNITION_STATE_MACHINE.md` - SR system design
- `STATE_MACHINE_TRACING_GUIDE.md` - SR execution tracing and implementation notes
...
### Operations
- `https://github.com/memphis-iis/mofacts/wiki/Remote-Install` - Deployment procedures
